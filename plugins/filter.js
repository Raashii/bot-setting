...
Rashi.tozara({pattern: 'filter ?(.*)', fromMe: true, desc: Lang.FILTER_DESC, dontAddCommandList: true}, (async (message, match) => {
...
Rashi.tozara({pattern: 'stop ?(.*)', fromMe: true, desc: Lang.STOP_DESC, dontAddCommandList: true}, (async (message, match) => {
    match = match[1].match(/[\'\"\“](.*?)[\'\"\“]/gsm);
    if (match === null) {
...
Rashi.tozara({on: 'text', fromMe: false }, (async (message, match) => {
    if(Config.BGMFILTER){
...
async function checkImAdmin(message, user = message.client.user.jid) {
    ...
    return sonuc.includes(true);
     ...
     Rashi.tozara({on: 'text', fromMe: false,onlyGroup: true}, (async (message, match) => {
...
if(pattern.test(message.message)){
    ...
    filtreler.map(
     ...
     Rashi.tozara({on: 'text', fromMe: false, onlyPm: true}, (async (message, match) => {
