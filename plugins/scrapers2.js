...
Asena.tozara({pattern: 'pint ?(.*)', fromMe: wk, desc: 'it send images from pintrest' }, async (message, match) => {
	
    
            if (match[1] === '') return await message.client.sendMessage(message.jid,"```Need word```");

    var webimage = await axios.get(`https://zenzapi.xyz/api/pinterest2?query=${match[1]}&apikey=Raashii`, { responseType: 'arraybuffer' })

  await message.client.sendMessage(message.jid,Buffer.from(webimage.data), MessageType.image, {mimetype: Mimetype.jpg})

});

    Asena.tozara({pattern: 'happymod ?(.*)', fromMe: wk, desc: 'it send mod apk links'}, async (message, match) => {
